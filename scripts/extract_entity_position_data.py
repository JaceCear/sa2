...
table_lengths = [0x88//4, 0x88//4, 0x88//4, 0x88//4] # SA2
...
# TODO
def print_asm_file(all_addresses, file_name_format):
    ...
    for i in range(len(all_addresses)):
        ...
        if (i > 0):
            prev_address = all_addresses[i-1][2]
        ...
        print(f"\t.incbin \"data/entity_positions/{label_str}.bin.rl\"")
        print()
...
with open('baserom.gba', 'rb') as rom:
    ...
    for entity_index in range(len(Entity)):
        ...
        for stage_index in range(table_lengths[entity_index]):
    ...
    print()
    print(os.name)
    #print_asm_file(all_addresses, export_directory)